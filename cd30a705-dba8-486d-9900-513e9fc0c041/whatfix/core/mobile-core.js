...
async function downloadConfigFromCDN(savedSegmentConfigMap) {
  const segmentationBaseUrl = `${configBaseUrl}/${appApiKey}${selfHostingEndPoints.SELF_HOSTING}/segmentation.json?v=${Math.random()}`;
  try {
    ...
    if (segmentationList?.segments?.length) {
      const data = await Promise.all(
        segmentationList.segments.map(async (segmentObject) => {
          const configUrl = `${configBaseUrl}/${appApiKey}${selfHostingEndPoints.SELF_HOSTING}${selfHostingEndPoints.SEGMENT_CONFIG_FOLDER}${segmentObject?.segmentId}.json?v=${Math.random()}`;
